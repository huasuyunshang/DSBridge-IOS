...
+ (NSArray *)allMethodFromClass:(Class)pclass {
    NSMutableArray *methods = [NSMutableArray array];
    while (pclass) {
        unsigned int count = 0;
        Method *method = class_copyMethodList(pclass, &count);
        ...
        Class cls = class_getSuperclass(pclass);
        pclass = [NSStringFromClass(cls) isEqualToString:NSStringFromClass([NSObject class])] ? nil : cls;
    }
...
+(NSString *)methodByNameArg:(NSInteger)argNum selName:(NSString *)selName class:(Class)pclass
{
    NSString *result = nil;
    if(pclass){
        NSArray *arr = [JSBUtil allMethodFromClass:pclass];
